...
class ValueHypothesisAgent:
    name = "ValueHypothesisAgent"
    model = "gpt-4.1"
    retry_policy = AgentRetryPolicy(max_attempts=2, backoff_ms=50, fallback_enabled=True)
    system_prompt = load_system_prompt("value_hypothesis_system.txt")
    ...
    def run(
        ...
        def _run(_attempt: int) -> tuple[dict, str, list[str]]:
            plan = run_tool_planned_agent(
                ...
                tool_specs=[
                    LLMToolSpec(
                        name="get_contacts",
                        description="Load contacts for the supplied account identifiers to ground the hypothesis.",
                        parameters={
                            "type": "object",
                            "properties": {
                                "account_ids": {
        ...
        def _fallback(error: Exception) -> tuple[dict, str, list[str]]:
            return {"hypotheses": []}, f"LLM agent failed ({error}).", ["fallback"]
        ...
        return ValueHypothesisOutput(
            hypotheses=[ValueHypothesisDraft.model_validate(item) for item in result_payload["hypotheses"]],
            estimate=estimate,
            trace=trace,
        )
