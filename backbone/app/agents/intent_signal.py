...
class IntentSignalAgent:
    ...
    system_prompt = load_system_prompt("intent_signal_system.txt")
    ...
    def run(
        ...
        def _run(_attempt: int) -> tuple[dict, str, list[str]]:
            plan = run_tool_planned_agent(
                ...
                tool_specs=[
                    LLMToolSpec(
                        name="get_signals",
                        description="Load intent and activity signals for the supplied account identifiers.",
                        parameters={
                            "type": "object",
                            "properties": {
                                "account_ids": {
                ...
                tool_handlers={
                    "get_signals": lambda arguments: [
                        ...
                        for item in self._tools.get_signals(
        ...
        def _fallback(error: Exception) -> tuple[dict, str, list[str]]:
            return {"assessments": []}, f"LLM agent failed ({error}).", ["fallback"]
        ...
        return IntentSignalOutput(
            assessments=[IntentAssessment.model_validate(item) for item in result_payload["assessments"]],
            estimate=estimate,
            trace=trace,
        )
